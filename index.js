...
import { EthersV6Provider } from "@irys/web-upload-ethereum-ethers-v6";
import { ethers } from "ethers";

window.irys = null;
window.lastFundAttemptAt = 0;
window.FUND_COOLDOWN_MS = 60000;
window.blobGcCache = new Map();
window.MAX_BLOB_CACHE_SIZE = 50;

window.InitSovereignIrys = async function(rawProvider) {
    const ethersProvider = new ethers.BrowserProvider(rawProvider);
    const irysUploader = await WebUploader(WebEthereum).withProvider(EthersV6Provider(ethersProvider));
    return irysUploader;
};
...
window.registerBlobForGc = function(url, objectUrl) {
    ...
    if (window.blobGcCache.has(url)) return;
    
    if (window.blobGcCache.size >= window.MAX_BLOB_CACHE_SIZE) {
        const oldestKey = window.blobGcCache.keys().next().value;
        const oldestObjUrl = window.blobGcCache.get(oldestKey);
        try { URL.revokeObjectURL(oldestObjUrl); } catch(e) {}
        window.blobGcCache.delete(oldestKey);
    }
    window.blobGcCache.set(url, objectUrl);
};
...
window.executeIrysUpload = async function(payload, tags, isFile = false, progressCtrl = null) {
    ...
    if (initialBalance.isLessThan(price)) {
        ...
        const msSinceLastFund = Date.now() - window.lastFundAttemptAt;
        if (msSinceLastFund < window.FUND_COOLDOWN_MS) {
            const waitSec = Math.ceil((window.FUND_COOLDOWN_MS - msSinceLastFund) / 1000);
        ...
        window.lastFundAttemptAt = Date.now();
...
window.executeIrysTopUp = async function(amountEthStr) {
    ...
    try {
        ...
        try {
            ...
            const feMsg = (fundErr && fundErr.message) || "";
            const isUserRejection = /user rejected|declined|denied|cancelled|user declined/i.test(feMsg);
            if (isUserRejection) throw fundErr; 
            fundCallFailed = true;
            window.appendLog(`[IRYS FUND WARNING] ${window.sanitizeForDOM(feMsg)} — กำลังตรวจสอบยอดเงินอีกครั้งก่อนสรุปว่าล้มเหลวจริง (ไม่ต้องกดซ้ำตอนนี้)`, "text-yellow-500");
        }
...
window.processDeedUploadAssignment = async function(event) {
    ...
    const isActuallyVideo = file.type.startsWith('video/');
    if ((userIntentType === 'video' && !isActuallyVideo) || (userIntentType === 'image' && isActuallyVideo)) {
        window.appendLog("SECURITY ALERT: โหมดที่เลือก (IMG/VID) ขัดแย้งกับประเภทไฟล์ที่อัปโหลด", "text-red-500 font-bold");
        window.showNotification("EXCEPTION: กรุณาอัปโหลดไฟล์ให้ตรงกับโหมดตัวเลือกบนแท่นอัปโหลด", "error");
        event.target.value = "";
        window.unlockSystem();
        return;
    }
    ...
    const env = window.detectUploadEnvironment();
    const guard = window.evaluateIosSafeguard(env, file, userIntentType === 'video' || isActuallyVideo);
    if (!guard.ok) {
